...
void weight_initialize(double *weight,int dim1,int dim2,double stddev,double mean)
{
    int i,j;
    double temp = 0;
    int count = 1;
    ...
    for (i=0;i<dim2;i++)
    {
        ...
        {
//            temp = rand() / 100000000;
//            weight[j+dim1*i] = (temp * stddev) + mean;
            weight[j+dim1*i] = -count;
            count++;
        }
    }
...
void sigmoid_activation(double *matrix,int dim1,int dim2)
{
    int i,j;
    for(i = 0;i<dim2;i++)
    {
        for(j = 0;j<dim1;j++)
        {
            matrix[j+dim1*i] = 1/(1+exp(matrix[j+dim1*i]));
        }
    }
}
